...
import { CalendarDays, ChevronLeft, Clock3, MapPin, Ticket, Users } from 'lucide-react'
...
import { generateBreadcrumbSchema, generateEventSchema } from '@/lib/seo/structured-data'
...
function mapApiEvent(event: ApiEventResponse): ProgramEvent {
  return {
    id: event.id,
    slug: event.slug,
    title: event.title,
...
export default function ProgramEvent() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (listRes.ok) {
          ...
          mappedList = (listJson?.events as ApiEventResponse[])?.map((item) => mapApiEvent(item)) ?? []
        }
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Kunne ikke hente arrangementet.')
          setEvent(null)
        }
      } finally {
        if (!cancelled) {
  ...
  const eventStructuredData = event
    ? generateEventSchema({
        ...
        endsAt: event.endsAt || null,
        venue: event.venue,
        venueAddress: event.venueInfo?.address || null,
        venueCity: event.venueInfo?.city || null,
  ...
  const breadcrumbData = event
    ? generateBreadcrumbSchema([
        { name: 'Hjem', url: getSiteUrl() },
        { name: 'Program', url: `${getSiteUrl()}/program` },
        { name: event.title, url: `${getSiteUrl()}/program/${event.slug}` },
      ])
    : null

  return (
    <div className='relative overflow-hidden'>
      {eventStructuredData && <StructuredData data={[eventStructuredData, breadcrumbData].filter(Boolean)} />}
      <BackgroundGlows />
...
function DetailRow({ icon: Icon, label, value, compact }: DetailRowProps) {
  return (
    <div className='flex gap-2'>
      <div className='mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-border/60 bg-secondary/30'>

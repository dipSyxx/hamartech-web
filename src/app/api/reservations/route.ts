...
function computeTicketExpirySeconds(
...
async function sendReservationTicketEmail(input: {
  ...
  const payload = {
    ...
    template_data: {
      ...
      reservationCreatedAt: input.reservation.createdAt.toISOString(),
      reservationUpdatedAt: input.reservation.updatedAt.toISOString(),
      ...
      isFree: input.event.isFree,
      requiresRegistration: input.event.requiresRegistration,
      startsAt: input.event.startsAt
        ? input.event.startsAt.toISOString()
        : null,
      endsAt: input.event.endsAt ? input.event.endsAt.toISOString() : null,
      ...
      reservations.map(async (reservation) => {
        ...
        const ticketToken = ticketCode
          ? createTicketToken({
              reservationId: reservation.id,
              ticketCode,
              exp: computeTicketExpirySeconds(
                reservation.event.endsAt,
